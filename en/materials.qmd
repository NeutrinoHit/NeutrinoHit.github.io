---
title: "Materials"
lang: en-US
toc: false
---

```{=html}
<div class="lecture-catalog">
  <p class="lecture-catalog-lead">
    NeutrinoHit library by material type: books, courses, talks, animations,
    code projects, photo archives, and films. This is a working map with direct entry points,
    not a list of links to other lists. Russian-only resources are marked explicitly.
  </p>

  <h2 class="section-inline-title">Books</h2>
  <section class="lecture-course-list" aria-label="Books catalog">
    <details class="lecture-course book-course-qft" open>
      <summary>
        <span class="lecture-course-status">Textbook · published in Russian</span>
        <span class="lecture-course-title">Quantum Field Theory for Experimentalists and More</span>
      </summary>
      <div class="lecture-course-body">
        <p>A practical two-volume textbook with examples, problems, and solutions. The Russian edition is published; the English edition is in preparation.</p>
        <div class="book-cover-spread" aria-label="Quantum field theory textbook cover">
          <img src="../assets/books/qft-cover-front.jpg" alt="Front cover of the quantum field theory textbook">
          <img src="../assets/books/qft-cover-back.jpg" alt="Back cover of the quantum field theory textbook">
        </div>
        <div class="lecture-material-list">
          <details class="lecture-material" open>
            <summary>Russian edition</summary>
            <div class="lecture-material-body">
              <p>The two Russian volumes are available in print and electronic formats.</p>
              <div class="lecture-material-list">
                <details class="lecture-material" open>
                  <summary>Print edition for Russia</summary>
                  <div class="lecture-material-body">
                    <div class="lecture-course-actions">
                      <a class="lecture-action-primary" href="https://shop.rcd.ru/catalog/fizika/20042/">Volume 1</a>
                      <a href="https://shop.rcd.ru/catalog/fizika/20097/">Volume 2</a>
                    </div>
                  </div>
                </details>
                <details class="lecture-material">
                  <summary>ROSPEX Publishing</summary>
                  <div class="lecture-material-body">
                    <div class="lecture-course-actions">
                      <a href="https://rospexpublishing.com/books/qft-volume-1.html?utm_source=neutrinohit&amp;utm_medium=website&amp;utm_campaign=qft_book">Volume 1</a>
                      <a href="https://rospexpublishing.com/books/qft-volume-2.html?utm_source=neutrinohit&amp;utm_medium=website&amp;utm_campaign=qft_book">Volume 2</a>
                      <a class="lecture-action-primary" href="https://rospexpublishing.com/books/qft-bundle.html?utm_source=neutrinohit&amp;utm_medium=website&amp;utm_campaign=qft_book">Bundle</a>
                    </div>
                  </div>
                </details>
                <details class="lecture-material">
                  <summary>Google Play Books</summary>
                  <div class="lecture-material-body">
                    <div class="lecture-course-actions">
                      <a class="lecture-action-primary" href="https://play.google.com/store/books/details?id=uLXTEQAAQBAJ">Volume 1</a>
                      <a href="https://play.google.com/store/books/details?id=KrbTEQAAQBAJ">Volume 2</a>
                    </div>
                  </div>
                </details>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>English edition</summary>
            <div class="lecture-material-body">
              <p>In preparation.</p>
            </div>
          </details>
        </div>
      </div>
    </details>

    <details class="lecture-course book-course-solar">
      <summary>
        <span class="lecture-course-status">Popular science · published in Russian</span>
        <span class="lecture-course-title">Solar Neutrino</span>
      </summary>
      <div class="lecture-course-body book-course-with-cover">
        <a class="book-cover-single" href="https://ast.ru/book/solnechnoe-neytrino-862573/">
          <img src="https://cdn.ast.ru/v2/ASE000000000862573/COVER/cover1__w340.jpg" alt="Solar Neutrino book cover">
        </a>
        <div>
          <p>A popular-science book for children about neutrinos, the Sun, and the big physics of tiny particles. The book is in Russian.</p>
          <div class="lecture-course-actions">
            <a class="lecture-action-primary" href="https://ast.ru/book/solnechnoe-neytrino-862573/">AST page (Russian)</a>
          </div>
        </div>
      </div>
    </details>

    <details class="lecture-course book-course-neutrino">
      <summary>
        <span class="lecture-course-status">In progress</span>
        <span class="lecture-course-title">Introduction to Neutrino Physics</span>
      </summary>
      <div class="lecture-course-body">
        <p>The book is in preparation. The working private repository is available to the author.</p>
        <div class="lecture-course-actions">
          <a href="https://github.com/dmitryvnaumov/NEUTRINObook">Private repository</a>
        </div>
      </div>
    </details>

    <details class="lecture-course book-course-particles">
      <summary>
        <span class="lecture-course-status">In progress</span>
        <span class="lecture-course-title">Introduction to Particle Physics</span>
      </summary>
      <div class="lecture-course-body">
        <p>The book is in preparation.</p>
      </div>
    </details>

    <details class="lecture-course book-course-stat">
      <summary>
        <span class="lecture-course-status">Online book · in progress</span>
        <span class="lecture-course-title">Statistical Analysis</span>
      </summary>
      <div class="lecture-course-body">
        <p>A practical online book on probability models, parameter estimation,
        hypothesis testing, and statistical inference in experimental physics.</p>
        <div class="lecture-course-actions">
          <a class="lecture-action-primary" href="../statistical-analysis-course/ru/book/index.html">Open book (Russian)</a>
          <a href="../statistical-analysis-course/ru/slides/applets.html">Applets (Russian)</a>
          <a href="../statistical-analysis-course/ru/slides/exercises/index.html">All exercises (Russian)</a>
          <a href="../statistical-analysis-course/ru/slides/exam_projects/index.html">Exam projects (Russian)</a>
        </div>
      </div>
    </details>
  </section>

  <h2 class="section-inline-title">Lecture Courses</h2>
  <section class="lecture-course-list" aria-label="Lecture catalog">
    <details id="qft" class="lecture-course lecture-course-qft" open>
      <summary>
        <span class="lecture-course-status">Published materials</span>
        <span class="lecture-course-title">Quantum Field Theory</span>
      </summary>
      <div class="lecture-course-body">
        <p>Courses on quantum field theory, the Standard Model, renormalization, accelerated frames, and selected QFT topics.</p>
        <div class="lecture-material-list">
          <details class="lecture-material" open>
            <summary>Textbook</summary>
            <div class="lecture-material-body">
              <p><em>Quantum Field Theory for Experimentalists and More</em> is a two-volume textbook with examples, problems, and solutions. The Russian edition is published; the English edition is in preparation.</p>
              <div class="book-cover-spread" aria-label="Quantum field theory textbook cover">
                <img src="../assets/books/qft-cover-front.jpg" alt="Front cover of the quantum field theory textbook">
                <img src="../assets/books/qft-cover-back.jpg" alt="Back cover of the quantum field theory textbook">
              </div>
              <div class="lecture-material-list">
                <details class="lecture-material" open>
                  <summary>Russian print edition</summary>
                  <div class="lecture-material-body">
                    <div class="lecture-course-actions">
                      <a class="lecture-action-primary" href="https://shop.rcd.ru/catalog/fizika/20042/">Volume 1</a>
                      <a href="https://shop.rcd.ru/catalog/fizika/20097/">Volume 2</a>
                    </div>
                  </div>
                </details>
                <details class="lecture-material">
                  <summary>ROSPEX Publishing</summary>
                  <div class="lecture-material-body">
                    <div class="lecture-course-actions">
                      <a href="https://rospexpublishing.com/books/qft-volume-1.html?utm_source=neutrinohit&amp;utm_medium=website&amp;utm_campaign=qft_book">Volume 1</a>
                      <a href="https://rospexpublishing.com/books/qft-volume-2.html?utm_source=neutrinohit&amp;utm_medium=website&amp;utm_campaign=qft_book">Volume 2</a>
                      <a class="lecture-action-primary" href="https://rospexpublishing.com/books/qft-bundle.html?utm_source=neutrinohit&amp;utm_medium=website&amp;utm_campaign=qft_book">Bundle</a>
                    </div>
                  </div>
                </details>
                <details class="lecture-material">
                  <summary>Google Play Books</summary>
                  <div class="lecture-material-body">
                    <div class="lecture-course-actions">
                      <a class="lecture-action-primary" href="https://play.google.com/store/books/details?id=uLXTEQAAQBAJ">Volume 1</a>
                      <a href="https://play.google.com/store/books/details?id=KrbTEQAAQBAJ">Volume 2</a>
                    </div>
                  </div>
                </details>
                <details class="lecture-material">
                  <summary>English edition</summary>
                  <div class="lecture-material-body">
                    <p>In preparation.</p>
                  </div>
                </details>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>Lecture course “Quantum Field Theory”</summary>
            <div class="lecture-material-body">
              <p>Basic QFT course: notes and video recordings are in Russian.</p>
              <div class="lecture-course-actions">
                <a href="https://teach-in.ru/file/synopsis/pdf/quantum-field-theory-naumov-MKv0.pdf">Notes (Russian)</a>
                <a href="https://www.youtube.com/playlist?list=PL28BG43DETPR-fdQGsm0qO2YhMWVsNa29">YouTube</a>
                <a href="https://rutube.ru/plst/1531108/">RuTube</a>
                <a href="https://vkvideo.ru/playlist/-235607329_5">VK</a>
                <a href="https://teach-in.ru/course/quantum-field-theory-naumov">Teach-in</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>Lecture course “Introduction to the Standard Model”</summary>
            <div class="lecture-material-body">
              <p>Introductory Standard Model course: notes and videos are in Russian.</p>
              <div class="lecture-course-actions">
                <a href="https://teach-in.ru/file/synopsis/pdf/introduction-to-the-standard-model-MKv0.pdf">Notes (Russian)</a>
                <a href="https://www.youtube.com/playlist?list=PL28BG43DETPT6A-H6fUtZvOJl0lv-YPZV">YouTube</a>
                <a href="https://rutube.ru/plst/1538123">RuTube</a>
                <a href="https://vkvideo.ru/playlist/-235607329_6">VK</a>
                <a href="https://teach-in.ru/course/introduction-to-the-standard-model">Teach-in</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>Lecture course “Renormalization in Quantum Field Theory”</summary>
            <div class="lecture-material-body">
              <p>Course notes and video playlists are in Russian.</p>
              <div class="lecture-course-actions">
                <a href="https://teach-in.ru/file/synopsis/pdf/renormalization-in-qft-M.pdf">Notes (Russian)</a>
                <a href="https://www.youtube.com/playlist?list=PL28BG43DETPT4L04sUPUNvZC93w4b60wG">YouTube</a>
                <a href="https://rutube.ru/plst/1530811">RuTube</a>
                <a href="https://vkvideo.ru/playlist/-235607329_4">VK</a>
                <a href="https://teach-in.ru/course/renormalization-in-qft">Teach-in</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>Accelerated frames and the Unruh effect</summary>
            <div class="lecture-material-body">
              <p>Four direct HTML slide lectures on accelerated frames, thermodynamics, and the Unruh effect.</p>
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../qft-lectures/ru/slides/Rindler.html">01. Accelerated frames (Russian)</a>
                <a href="../qft-lectures/ru/slides/Thermodynamics.html">02. Thermodynamics (Russian)</a>
                <a href="../qft-lectures/ru/slides/UnruhEffect-1.html">03. Unruh effect I (Russian)</a>
                <a href="../qft-lectures/ru/slides/UnruhEffect-2.html">04. Unruh effect II (Russian)</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>Attraction and Repulsion in Quantum Field Theory</summary>
            <div class="lecture-material-body">
              <p>Lecture on scalar QED and wave packets: how attraction and repulsion are encoded in QFT.</p>
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../qft-lectures/en/slides/ScalarQEDAttractionRepulsion.html">HTML slides</a>
              </div>
            </div>
          </details>
        </div>
      </div>
    </details>

    <details id="stat" class="lecture-course lecture-course-stat">
      <summary>
        <span class="lecture-course-status">15 English lectures · 13 Russian lectures</span>
        <span class="lecture-course-title">Statistical Analysis of Data</span>
      </summary>
      <div class="lecture-course-body">
        <p>A practical course for high-energy physics, astrophysics, and experimental data analysis. English-language lectures are listed directly below; Russian-only supporting materials are marked.</p>
        <div class="lecture-course-actions">
          <a class="lecture-action-primary" href="../statistical-analysis-course/ru/book/index.html">Online book (Russian)</a>
          <a href="../statistical-analysis-course/ru/slides/applets.html">Applets (Russian)</a>
          <a href="../statistical-analysis-course/ru/slides/exercises/index.html">All exercises (Russian)</a>
          <a href="../statistical-analysis-course/ru/slides/exam_projects/index.html">Exam projects (Russian)</a>
        </div>
        <div class="lecture-material-list">
          <details class="lecture-material">
            <summary>01. Introduction</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-01-introduction.html">Open slides</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>02. Probability and Bayes</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-02-probability-bayes.html">Open slides</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>03. Random variables and densities</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-03-random-variables-and-densities.html">Open slides</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>04. Expectation and error propagation</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-04-expectation-and-error-propagation.html">Open slides</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>05. Common probability distributions</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-05-common-probability-distributions.html">Open slides</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>06. Monte Carlo method</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-06-monte-carlo-method.html">Open slides</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>07. Statistical tests basics</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-07-statistical-tests-basics.html">Open slides</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>08. Test statistics and multivariate methods</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-08-test-statistics-and-multivariate-methods.html">Open slides</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>09. Goodness of fit and significance</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-09-goodness-of-fit-and-significance.html">Open slides</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>10. Feldman-Cousins and CLs</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-10-feldman-cousins-and-cls.html">Open slides</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>11. Parameter estimation and maximum likelihood</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-11-parameter-estimation-and-maximum-likelihood.html">Open slides</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>12. Method of least squares</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-12-method-of-least-squares.html">Open slides</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>13. Interval estimation and limits</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-13-interval-estimation-and-limits.html">Open slides</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>14. Nuisance parameters and systematics</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-14-nuisance-parameters-and-systematics.html">Open slides</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>15. Bayesian examples</summary>
            <div class="lecture-material-body">
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../statistical-analysis-course/en/slides/lecture-15-bayesian-examples.html">Open slides</a>
              </div>
            </div>
          </details>
        </div>
      </div>
    </details>

    <details id="neutrino" class="lecture-course lecture-course-neutrino">
      <summary>
        <span class="lecture-course-status">Courses and master classes</span>
        <span class="lecture-course-title">Neutrino Physics</span>
      </summary>
      <div class="lecture-course-body">
        <p>Introductory neutrino lectures and the solar-neutrino masterclass are listed directly here.</p>
        <div class="lecture-material-list">
          <details class="lecture-material">
            <summary>Introduction to neutrino physics</summary>
            <div class="lecture-material-body">
              <p>Three introductory HTML slide lectures.</p>
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../neutrinophysics/introduction/Introduction/index.html">01. Introduction</a>
                <a href="../neutrinophysics/introduction/Parity/index.html">02. Parity</a>
                <a href="../neutrinophysics/introduction/Helicity/index.html">03. Helicity</a>
              </div>
            </div>
          </details>
          <details class="lecture-material" open>
            <summary>Solar neutrino masterclass</summary>
            <div class="lecture-material-body">
              <p>Sources of solar neutrinos, oscillations, MSW physics, detector statistics, and an SK-like fit project. Lectures, project materials, and datasets are opened from one tab.</p>
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../neutrinophysics/solar-neutrino-masterclass/slides/00_solar_neutrino_physics.html">00. Solar Neutrinos and Oscillations</a>
                <a href="../neutrinophysics/solar-neutrino-masterclass/slides/00_solar_neutrino_physics_appendix.html">00. Appendix</a>
                <a href="../neutrinophysics/solar-neutrino-masterclass/slides/01_solar_sources.html">01. Fluxes, event rates, normalization fit</a>
                <a href="../neutrinophysics/solar-neutrino-masterclass/slides/02_msw_detector_statistics.html">02. Oscillations and fit</a>
                <a href="../neutrinophysics/solar-neutrino-masterclass/assignments/sk_integrated_project.html">Project: SK-like fit</a>
                <a href="../neutrinophysics/solar-neutrino-masterclass/data/masterclass1/masterclass1_data.zip">Masterclass 1 data</a>
                <a href="../neutrinophysics/solar-neutrino-masterclass/data/masterclass2/masterclass2_data.zip">Masterclass 2 data</a>
                <a href="../neutrinophysics/solar-neutrino-masterclass/data/project/solar_neutrino_project_data.zip">Project data</a>
              </div>
            </div>
          </details>
        </div>
      </div>
    </details>

    <details id="particles" class="lecture-course lecture-course-particles">
      <summary>
        <span class="lecture-course-status">Mini-course and course materials</span>
        <span class="lecture-course-title">Particle Physics</span>
      </summary>
      <div class="lecture-course-body">
        <p>The particle-physics mini-course and the larger course materials are listed directly here.</p>
        <div class="lecture-material-list">
          <details class="lecture-material" open>
            <summary>Mini-course: introduction to particle physics</summary>
            <div class="lecture-material-body">
              <p>Two introductory lectures, a formula sheet, questions, and practical notebooks on cross sections, decays, and neutrino oscillations.</p>
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../particlephysics/IntroParticlePhysicsBaikal/slides/01_particles_fields_interactions.html">01. Particles, Fields, and Interactions</a>
                <a href="../particlephysics/IntroParticlePhysicsBaikal/slides/02_standard_model_neutrinos.html">02. The Standard Model and Neutrinos</a>
                <a href="../particlephysics/IntroParticlePhysicsBaikal/handouts/formula_sheet.html">Formula sheet</a>
                <a href="../particlephysics/IntroParticlePhysicsBaikal/handouts/01_questions_particles_interactions.html">Questions after lecture 1</a>
                <a href="../particlephysics/IntroParticlePhysicsBaikal/handouts/02_questions_standard_model_neutrinos.html">Questions after lecture 2</a>
                <a href="https://neutrinohit.github.io/particlephysics/IntroParticlePhysicsBaikal/notebooks/01_event_rates_and_cross_sections.ipynb">Notebook: rates and cross sections</a>
                <a href="https://neutrinohit.github.io/particlephysics/IntroParticlePhysicsBaikal/notebooks/02_decay_law_and_lifetime.ipynb">Notebook: decay law</a>
                <a href="https://neutrinohit.github.io/particlephysics/IntroParticlePhysicsBaikal/notebooks/03_neutrino_oscillation_probability.ipynb">Notebook: neutrino oscillations</a>
              </div>
            </div>
          </details>
          <details class="lecture-material">
            <summary>Full course: introduction to particle physics</summary>
            <div class="lecture-material-body">
              <p>English layout of the full course: course slides and course book.</p>
              <div class="lecture-course-actions">
                <a class="lecture-action-primary" href="../particlephysics/ParticlePhysics/en/slides/index.html">Course slides</a>
                <a href="../particlephysics/ParticlePhysics/en/book/index.html">Course book</a>
              </div>
            </div>
          </details>
        </div>
      </div>
    </details>

    <details id="gravity" class="lecture-course lecture-course-gravity">
      <summary>
        <span class="lecture-course-status">Planned</span>
        <span class="lecture-course-title">Gravity</span>
      </summary>
      <div class="lecture-course-body">
        <p>Future course section.</p>
      </div>
    </details>

    <details id="oscillations" class="lecture-course lecture-course-oscillations">
      <summary>
        <span class="lecture-course-status">Planned</span>
        <span class="lecture-course-title">Neutrino oscillation theory in QFT</span>
      </summary>
      <div class="lecture-course-body">
        <p>Future specialized course on quantum-field descriptions of neutrino oscillations.</p>
      </div>
    </details>
  </section>

  <h2 class="section-inline-title">Talks</h2>
  <section class="lecture-course-list" aria-label="Talks">
    <details class="lecture-course lecture-course-talk-conferences" open>
      <summary>
        <span class="lecture-course-status">Conference · 2026</span>
        <span class="lecture-course-title">Status and Prospects of Neutrino Physics</span>
      </summary>
      <div class="lecture-course-body">
        <p>Talk at the June 2-5, 2026 conference: Russian projects in the global context.</p>
        <div class="lecture-course-actions">
          <a class="lecture-action-primary" href="../talks/protvino-2026/dnaumov.html">Talk slides (Russian)</a>
          <a href="https://indico.ihep.su/e/PPIHE2026">Conference page (Russian)</a>
        </div>
      </div>
    </details>
  </section>

  <h2 class="section-inline-title">Animations</h2>
  <section class="lecture-course-list" aria-label="Animations">
    <details class="lecture-course resource-card-animation" open>
      <summary>
        <span class="lecture-course-status">28 stories · 72 clips</span>
        <span class="lecture-course-title">Scientific animations by dvnanima</span>
      </summary>
      <div class="lecture-course-body">
        <p>Neutrinos and particles, fields and waves, mathematics and dynamical systems, and visual science stories.</p>
        <div class="lecture-course-actions">
          <a class="lecture-action-primary" href="../animations.html">Open gallery (Russian labels)</a>
          <a href="https://github.com/NeutrinoHit/dvnanima">Source code</a>
          <a href="../animations.html#neutrino-particles">Neutrinos and particles (Russian labels)</a>
          <a href="../animations.html#fields-waves">Fields and waves (Russian labels)</a>
          <a href="../animations.html#math-dynamical-systems">Mathematics (Russian labels)</a>
        </div>
      </div>
    </details>
  </section>

  <h2 class="section-inline-title">Code and Models</h2>
  <section class="lecture-course-list" aria-label="Code and models">
    <details class="lecture-course software-course-tools" open>
      <summary>
        <span class="lecture-course-status">Tools</span>
        <span class="lecture-course-title">dvnanima and gTracker</span>
      </summary>
      <div class="lecture-course-body">
        <p>Tools for scientific animations and animated application maps by city.</p>
        <div class="lecture-course-actions">
          <a class="lecture-action-primary" href="https://github.com/NeutrinoHit/dvnanima">dvnanima</a>
          <a href="https://github.com/NeutrinoHit/gTracker">gTracker</a>
        </div>
      </div>
    </details>
    <details class="lecture-course software-course-models">
      <summary>
        <span class="lecture-course-status">Physical models</span>
        <span class="lecture-course-title">TDSE-Diffraction and neutrino-nucleus-coherence</span>
      </summary>
      <div class="lecture-course-body">
        <p>Numerical calculations and demonstration models for physical processes.</p>
        <div class="lecture-course-actions">
          <a class="lecture-action-primary" href="https://github.com/NeutrinoHit/TDSE-Diffraction">TDSE-Diffraction</a>
          <a href="https://github.com/NeutrinoHit/neutrino-nucleus-coherence">neutrino-nucleus-coherence</a>
        </div>
      </div>
    </details>
  </section>

  <h2 class="section-inline-title">Photos and Films</h2>
  <section class="lecture-course-list" aria-label="Photos and films">
    <details class="lecture-course photo-course-archive" open>
      <summary>
        <span class="lecture-course-status">2 albums</span>
        <span class="lecture-course-title">NeutrinoHit photo archive</span>
      </summary>
      <div class="lecture-course-body">
        <p>Photos from lectures, scientific schools, trips, and meetings.</p>
        <div class="lecture-course-actions">
          <a class="lecture-action-primary" href="../albums/v-a-rubakov-baikal-school-2009/">V. A. Rubakov, Baikal 2009</a>
          <a href="../albums/10-17-may-2026/">Chinese colleagues visit, 2026</a>
        </div>
      </div>
    </details>
    <details class="lecture-course cinema-course-documentary">
      <summary>
        <span class="lecture-course-status">2 projects</span>
        <span class="lecture-course-title">Films and animated films</span>
      </summary>
      <div class="lecture-course-body">
        <div class="lecture-course-actions">
          <a class="lecture-action-primary" href="https://youtu.be/x--KvzTlvL0?si=ubRc5QdrKFSxNQui">The Universe in a Neutrino Telescope</a>
          <a href="https://youtu.be/Qja-x8WjYiw?si=LY2cQp5rX1U076sg">Nerpa and the Mystery of Baikal</a>
        </div>
      </div>
    </details>
  </section>
</div>
```
